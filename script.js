...
// --- Fonctions Puppeteer (fillField, humanScrollToClaim, etc.) identiques aux versions précédentes ---
// (je les omet pour raccourcir, mais elles doivent être présentes)
...
async function saveAccounts(accounts, modifiedAccount = null) {
    ...
    while (maxRetries-- > 0) {
        try {
            ...
            if (e.status === 409) {
                console.warn(`⚠️ Conflit 409, nouvel essai...`);
...
(async () => {
    try {
        ...
        const targetAccount = accounts.find(acc =>
            acc.email === CLAIM_EMAIL && acc.platform === CLAIM_PLATFORM && acc.enabled !== false
        );
        if (!targetAccount) {
