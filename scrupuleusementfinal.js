...
// Fonctions utilitaires (fillField, humanClickAt, performLogin) inchangées
...
// Fonction parseProxyUrl modifiée pour SOCKS5
function parseProxyUrl(proxyUrl) {
    if (!proxyUrl) return null;
    // Format HTTP
    const matchHttp = proxyUrl.match(/^http:\/\/([^:]+):([^@]+)@([^:]+):(\d+)$/);
    if (matchHttp) {
        return {
            type: 'http',
            server: `http://${matchHttp[3]}:${matchHttp[4]}`,
            username: matchHttp[1],
            password: matchHttp[2]
        };
    }
    // Format SOCKS5
    const matchSocks = proxyUrl.match(/^socks5:\/\/([^:]+):([^@]+)@([^:]+):(\d+)\/?$/);
    if (matchSocks) {
        return {
            type: 'socks5',
            server: `socks5://${matchSocks[3]}:${matchSocks[4]}`,
            username: matchSocks[1],
            password: matchSocks[2]
        };
    }
...
// --- Main avec réessai et support SOCKS5 ---
async function run() {
    let browser;
    try {
